...
class AdminPanelController extends Controller
{
    ...
    function uploadProfile(Request $request)
    {
        ...
        try {
            ...
            if ($restaurant === null) {
                // Profile not found, create a new instance
                $restaurant = new RestaurantProfile();
            } else {
                // Profile found, delete the previous image if it exists
                $source2 = public_path('uploads/profile/'.$restaurant->logo);
            ...
            if ($file = request()->file('image')) {
                ...
                $extra_upload_path = 'uploads/profile/';
                $file->move($extra_upload_path, $id_proof);
                $restaurant->logo = $id_proof;
            }
    ...
    function deleteCategory($id)
    {
        try {
            ...
            if ($category == null) {
                // echo 'no record';
                return response('No data found with this Id', 400);
            }
            try {
                $source = public_path('uploads/categories/'.$category->image);
                ...
                if ($result) {
                    ...
                    return response('Data can not be deleted', 420);
                }
            } catch (Exception $e) {
                // echo 'error while deleting';
                return response('Data can not be deleted', 500);
            }
        } catch (Exception $e) {
            // echo 'error while finding';
            return response('Error while finding', 500);
        }
    }
    ...
    function category_updated(Request $request)
    {
        ...
        try {
            ...
            if($file=request()->hasFile('image'))
            {
                ...
                $source2 = public_path('uploads/categories/'.$category->image);
                if (\File::exists($source2)) {
                    \File::delete($source2);
                }
                ...
                $file = request()->file('image');
                $id_proof = time().'.'.strtolower($file->getClientOriginalExtension());
                $extra_upload_path = 'uploads/categories/';
                $file->move($extra_upload_path,$id_proof);
                $category->image = $id_proof;
            }
        ...
        try {
            ...
            try {
                ...
                if($file=request()->hasFile('image'))
                {
                    ...
                    $source2 = public_path('uploads/products/'.$product->image);
                    if (\File::exists($source2)) {
                        \File::delete($source2);
                    }
                    ...
                    $file = request()->file('image');
                    $id_proof = time().'.'.strtolower($file->getClientOriginalExtension());
                    $extra_upload_path = 'uploads/products/';
                    $file->move($extra_upload_path,$id_proof);
                    $product->image = $id_proof;
                }
